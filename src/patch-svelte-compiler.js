/*
  ...
  pnpm add -D estree-walker spread concat acorn magic-string tosource fs
...
if (fs.existsSync(input_file) || fs.existsSync(output_file)) {
	console.log(
		"error: input or output file exists. run this script only once"
	);
	process.exit(1);
}
...
estree_walk(ast, {
	enter: function (node) {
		// enter: function ( node, _parent, _prop, _index ) {
		...
		if (
			node.arguments.find((a) => a.type === "SpreadElement") === undefined
		) {
			return;
		}
		...
		if (replaceMethod === "spread") {
			...
			const pushPropLen = content
				.substring(pushProp.start, node.end)
				.indexOf("(");
		...
		if (replaceMethod === "concat") {
			...
			node.arguments.forEach((a) => {
				if (a.type === "SpreadElement") {
					...
					const spreadArgSrc = content.substring(
						a.argument.start,
						a.argument.end
					);
