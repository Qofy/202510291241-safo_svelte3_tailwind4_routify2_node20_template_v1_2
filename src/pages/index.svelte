<script>
  ...
  let openFiles = []; // Array of {path, name, content, modified}
  let currentFile = null;

  // Declare at top level so accessible everywhere
  let editor, ws, currentPath = null;
  let terminal, fitAddon, termStarted = false;
  let statusEl, wsPathEl, logEl, termEl, filesEl, xtermEl, previewEl;

  // Move all functions that need to be accessed outside onMount to top level
  function log(el, s) { 
    if (el) {
      el.textContent += s + "\n"; 
  ...
  function setWorkspacePath() { 
    if (wsPathEl) wsPathEl.textContent = location.host; 
  }

  function renderMarkdown() {
    if (!currentPath || !editor || !previewEl) return;
  ...
  async function openFile(p) {
    ...
    if (js.ok) {
      ...
      const existingFile = openFiles.find(f => f.path === p);
      if (!existingFile) {
      ...
      if (editor) {
        ...
        if (lang === 'markdown' && previewEl && !previewEl.classList.contains('hidden')) {
          renderMarkdown();
        }
      }
    } else { 
      log(logEl, 'open failed: ' + js.error); 
    }
  }
  ...
  function handleFileClose(path) {
    ...
    if (path === currentPath) {
      if (openFiles.length > 0) {
        const nextFile = openFiles[fileIndex] || openFiles[fileIndex - 1];
        openFile(nextFile.path);
      } else {
        currentPath = null;
        currentFile = null;
        if (editor) editor.setValue('// Select a file on the left to open it.');
      }
    }
  }
  ...
  onMount(()=>{
    ...
    document.getElementById('runner-start').onclick = () => { 
      if (ws && ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify({ type:'runner_start' })); 
    }
    document.getElementById('runner-restart').onclick = () => { 
      if (ws && ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify({ type:'runner_restart' })); 
    }
    document.getElementById('runner-stop').onclick = () => { 
      if (ws && ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify({ type:'runner_stop' })); 
    }
    ...
    // Track editor changes to mark as modified
    editor.onDidChangeModelContent(() => {
      if (currentPath) {
        ...
        if (currentPath.endsWith('.md') || currentPath.endsWith('.markdown')) {
